...
type ScalingAdviceRequest struct {
	...
	CreationTime time.Time `json:",omitzero"`
	...
	Feedback *sacorev1alpha1.ScalingFeedback `json:",omitempty"`
	// Constraint represents the constraints using which the scaling advice is generated.
	Constraint *sacorev1alpha1.ScalingConstraint `json:",omitempty"`
...
type ScalingAdviceResponse struct {
	...
	ScalingAdvice *sacorev1alpha1.ScalingAdvice `json:",omitempty"`
...
//     advice generation. No further results will be sent for the associated request.
...
type PodInfo struct {
	...
	AggregatedRequests corev1.ResourceList `json:",omitempty"`
	// NodeSelector is the node selector for the Pod.
	NodeSelector map[string]string `json:",omitempty"`
	// Affinity is the affinity rules for the Pod.
	Affinity *corev1.Affinity    `json:",omitempty"`
	Overhead corev1.ResourceList `json:",omitempty"`
	...
	SchedulerName string `json:",omitempty"`
	// PriorityClassName is the name of the priority class that should be used to schedule the Pod.
	PriorityClassName string                  `json:",omitempty"`
	PreemptionPolicy  corev1.PreemptionPolicy `json:",omitempty"`
	RuntimeClassName  string                  `json:",omitempty"`
	BasicMeta
...
type BasicMeta struct {
	...
	DeletionTimestamp time.Time `json:",omitzero"`
	// OwnerReferences are the owner references associated with the resource.
	OwnerReferences []metav1.OwnerReference `json:",omitempty"`
}
...
// ScaleOutSimulator is a facade that executes simulations to generate ScaleOutPlans. Implementations vary depending on the commontypes.SimulatorStrategy used.
...
// the SimulatorStrategy, we may run independent multiple simulations differentiated by scaling a node for a combination
