...
func (r *RunState) Init(parentCtx context.Context, name string, runNum uint32, view minkapi.View, traceDir string) (context.Context, error) {
	...
	if logutil.VerbosityFromContext(r.ctx) > 3 {
		_ = viewutil.LogDumpObjects(r.ctx, "RunState.Init", view)
	}
...
func (r *RunState) createNode(nodeTemplate plannerapi.ScaleOutNodeTemplate) (*corev1.Node, error) {
	...
	if logutil.VerbosityFromContext(r.ctx) >= viewutil.DefaultDumpVerbosity {
		yamlDumpPath, err := objutil.SaveRuntimeObjAsYAMLToPath(node, r.traceDir, node.Name+".yaml")
		if err != nil {
			return nil, err
		}
		log.V(viewutil.DefaultDumpVerbosity).Info("dumped ScaleOutSimNode YAML", "scaleOutSimNodeName", node.Name, "yamlDumpPath", yamlDumpPath)
	}
...
func (r *RunState) createCSINode(storageMetaAccess plannerapi.StorageMetaAccess, scaleOutSimNode *corev1.Node) error {
	...
	if logutil.VerbosityFromContext(r.ctx) >= viewutil.DefaultDumpVerbosity {
		yamlDumpPath, err := objutil.SaveRuntimeObjAsYAMLToPath(csiNode, r.traceDir, "csi-"+scaleOutSimNode.Name+".yaml")
		if err != nil {
			return err
		}
		log.V(viewutil.DefaultDumpVerbosity).Info("dumped ScaleOutSimCSINode YAML", "csiNodeName", csiNode.Name, "yamlDumpPath", yamlDumpPath)
	}
	return err
}
